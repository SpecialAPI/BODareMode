...
namespace BODareMode
{
    ...
    public static class ILTools
    {
        ...
        /// <param name="name">The field's name.</param>
        /// <returns>The field.</returns>
        public static FieldInfo EnumeratorField(this Type tp, string name) => tp.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic).First(x => x != null && x.Name != null && (x.Name.Contains($"<{name}>") || x.Name.Contains($"__{name}") || x.Name == name));
    }
}
